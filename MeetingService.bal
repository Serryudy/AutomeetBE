...
service /api on ln {
    ...
    resource function post direct/meetings(http:Request req) returns Meeting|ErrorResponse|error {
        ...
        foreach MeetingParticipant participant in participants {
            ...
            map<json> userFilter = { "username": participant.username };
            record {}|() userRecord = check mongodb:userCollection->findOne(userFilter);

            if userRecord is () {
                // Unregistered user
                string externalUuid = uuid:createType1AsString();
                string meetingLink = "";

                if (meeting.meetingType == "group" || meeting.meetingType == "round_robin") {
                    meetingLink = string `http://localhost:3000/exavailability/${externalUuid}/${meetingId}`;
                } else if (meeting.meetingType == "direct") {
                    meetingLink = string `http://localhost:3000/exmeetingdetails/${meetingId}`;
                }

                // You need the participant's email (should be in MeetingParticipant)
                if participant.email != "" {
                    // Implement this function to send the email
                    error? emailResult = sendExternalParticipantEmail(participant.email, meeting.title, meetingLink);
                    if emailResult is error {
                        log:printError("Failed to send external participant email", emailResult);
                    }
                }
                // Optionally, store the uuid and email mapping in a collection for future reference
            }
        }
